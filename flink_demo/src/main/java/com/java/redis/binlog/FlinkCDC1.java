...
//        //1.1Checkpoint相关
//        */
///*读取的是binlog中的数据，如果集群挂掉，尽量能实现断点续传功能。如果从最新的读取（丢数据）。如果从最开始读（重复数据）。理想状态：读取binlog中的数据读一行，保存一次读取到的（读取到的行）位置信息。而flink
//        中读取行位置信息保存在Checkpoint中。使用Checkpoint可以把flink中读取（按行）的位置信息保存在Checkpoint中*//*
//
//        env.enableCheckpointing(5000L);//5s执行一次Checkpoint
//        //设置Checkpoint的模式：精准一次
//        env.getCheckpointConfig().setCheckpointingMode(CheckpointingMode.EXACTLY_ONCE);
//        //任务挂掉的时候是否清理checkpoint。使任务正常退出时不删除CK内容，有助于任务恢复。默认的是取消的时候清空checkpoint中的数据。RETAIN_ON_CANCELLATION
//        // 表示取消任务的时候，保存最后一次的checkpoint。便于任务的重启和恢复，正常情况下都使用RETAIN
...
//        //设置一个状态后端 jobManager。如果使用的yarn集群模式，jobManager随着任务的生成而生成，任务挂了jobManager就没了。因此需要启动一个状态后端。只要设置checkpoint
//        // ，尽量就设置一个状态后端。保存在各个节点都能读取的位置：hdfs中
...
//                .hostname("localhost") //指定监控的哪台服务器（MySQL安装的位置）
//                .port(3306) //MySQL连接的端口号
//                .username("root") //用户
//                .password("root")//密码
//                .databaseList("test_sql") //list：可以监控多个库
//                .tableList("test_sql.a") //如果不写则监控库下的所有表，需要使用【库名.表名】
//                //debezium中有很多配置信息。可以创建一个对象来接收
//                //.debeziumProperties(properties)
//                .deserializer(new StringDebeziumDeserializationSchema()) //读的数据是binlog文件，反序列化器，解析数据
//                .startupOptions(StartupOptions.initial()) //初始化数据：空值读不读数据库中的历史数据。initial（历史+连接之后的）、latest-offset
//                // （连接之后的）。timestamp（根据指定时间戳作为开始读取的位置）
//                .build();
//
